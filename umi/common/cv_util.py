...
def detect_localize_aruco_tags(
    ...
    # corners: 检测到的aruco的四个角点的(x,y)坐标；顺序为：左上-右上-右下-左下
    # ids: 检测到的aruco的id
    ...
    for this_id, this_corners in zip(ids, corners):
        ...
        # 返回的是marker在camera坐标系下的位姿
        # 4.7版本后的opencv使用solvePnP代替这个函数
...
if __name__ == "__main__":
    img = np.zeros((2028, 2704), dtype=np.uint8)
    mask = draw_predefined_mask(img, color=255, mirror=False, gripper=False, finger=True, use_aa=False)
